...
public class ButtonLoad extends JLabel {
	...
	public ButtonLoad(MenuFrame menuFrame, GameSetting gameSetting) {
		 ...
		 * 
		 * PositionBoard positionBoard = new PositionBoard(gameSetting.getLevel(),
		 * gameSetting); positionBoard.setBoardPanel(mFrame.getBoardPanel()); for (int i
		 * = 0; i < 63; i++) { positionBoard.getHashMapPieces().put(i, "NoPiece"); }
		 * String workingdir = System.getProperty("user.dir"); JFileChooser fileChooser
		 * = new JFileChooser(new File(workingdir)); FileNameExtensionFilter filter =
		 * new FileNameExtensionFilter("TEXT FILES", "txt", "text");
		 * fileChooser.addChoosableFileFilter(filter);
		 * 
		 * fileChooser.setFileFilter(filter); int returnVal =
		 * fileChooser.showOpenDialog(new JFrame()); if (returnVal ==
		 * JFileChooser.APPROVE_OPTION) { File file = fileChooser.getSelectedFile(); try
		 * { BufferedReader br = new BufferedReader(new FileReader(file)); String line =
		 * br.readLine(); while (line != null) { Pieces pieces =
		 * Pieces.getPiecesByText(line, positionBoard); if
		 * (pieces.getSide().equals("Down")) {
		 * positionBoard.getListPiecesHuman().add(pieces);
		 * positionBoard.getHashMapPieces().put(pieces.getNumberInBoard(), "Human"); }
		 * else { positionBoard.getListPiecesAi().add(pieces);
		 * positionBoard.getHashMapPieces().put(pieces.getNumberInBoard(), "Ai");
		 * 
		 * } line = br.readLine(); } positionBoard.setAiWasEat(false);
		 * positionBoard.setHumanWasEat(false);
		 * 
		 * mFrame.getBoardPanel().setPositionBoard(positionBoard);
		 * mFrame.getBoardPanel().repaint();
		 * 
		 * menuFrame.setVisible(false); mFrame.setVisible(true); br.close(); } catch
